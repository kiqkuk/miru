...
void Bar::setLayout(const std::string& layout)
{
	...
	_showLayout = true;
	_layoutChangedAt = std::chrono::steady_clock::now();
	invalidate();
}
...
void Bar::render()
{
	...
	if (_showLayout) {
		...
		if (duration_cast<milliseconds>(now - _layoutChangedAt).count() > 800) {
			_showLayout = false;
		}
	}
	...
	if (_showLayout) {
		renderComponent(_layoutCmp, _showLayout);
	}
...
void Bar::renderComponent(BarComponent& component, bool visible)
{
